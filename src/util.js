export function questionAnsweredByCurrentUser(question, currentUser) {
    if (question != null) {
        return (
            question.optionOne.votes.filter(vote => vote === currentUser)
                .length > 0 ||
            question.optionTwo.votes.filter(vote => vote === currentUser)
                .length > 0
        );
    }
}
...
export function getResultsFromQuestion(question) {
    if (question != null) {
        ...
        if (votesSum > 0) {
            var optionOnePercentage =
                ((optionOneUserCount / votesSum) * 100).toFixed(2) + "%";
            var optionTwoPercentage =
                ((optionTwoUserCount / votesSum) * 100).toFixed(2) + "%";
        } else {
